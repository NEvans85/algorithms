...
def sumSubsets(arr, num):
    ...
    def findSubsets(part, startIdx):
        partSum = sum(part)
        for idx, el in enumerate(arr[startIdx:]):
            if el < num - partSum:
                newPart = part[:]
                newPart.append(el)
                findSubsets(newPart, startIdx + idx + 1)
            elif el == num - partSum:
                completeSubset = part[:]
                completeSubset.append(el)
                if completeSubset not in subsets:
                    subsets.append(completeSubset)
