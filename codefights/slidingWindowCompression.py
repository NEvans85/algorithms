...
def losslessDataCompression(inputString, width):
    ...
    while i < len(inputString):
        wStart = max(0, i - width)
        if inputString[i] in inputString[wStart:i]:
            possStartIdxs = [idx + wStart for idx, ch in enumerate(inputString[wStart:i]) if ch == inputString[i]]
            j = 1
            while i + j < len(inputString) and j < len(inputString[wStart:i]):
                testIdxs = possStartIdxs[:]
                validIdxs = [idx for idx in possStartIdxs if inputString[i+j] == inputString[idx + j]]
                if len(validIdxs) > 0:
                    possStartIdxs = validIdxs
                    j += 1
                else:
                    break
            startIdx = possStartIdxs[0]
            cString += '(' + str(startIdx) + ',' + str(j) + ')'
            i += j
        else:
