...
def dailyOHLC(timestamp, instrument, side, price, size):
    results = []
    dateRanges = [[0]]
    currDate = date.fromtimestamp(timestamp[0])
    for idx, ts in enumerate(timestamp):
        nextDate = date.fromtimestamp(ts)
        if nextDate != currDate:
            dateRanges[-1].append(idx)
            dateRanges.append([idx])
            currDate = date.fromtimestamp(ts)
    dateRanges[-1].append(len(timestamp))
    for dr in dateRanges:
        instPriceDict = defaultdict(list)
        for idx in range(dr[0], dr[1]):
            instPriceDict[instrument[idx]].append(price[idx])
        dateInsts = sorted(instPriceDict.keys())
        for inst in dateInsts:
            OHLC = []
            OHLC.append(date.fromtimestamp(timestamp[dr[0]]).isoformat())
            OHLC.append(inst)
            OHLC.append('{0:.2f}'.format(instPriceDict[inst][0]))
            OHLC.append('{0:.2f}'.format(max(instPriceDict[inst])))
            OHLC.append('{0:.2f}'.format(min(instPriceDict[inst])))
            OHLC.append('{0:.2f}'.format(instPriceDict[inst][-1]))
            results.append(OHLC)
    return results
